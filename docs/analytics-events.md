...
- **Nerede**:
  - `src/modules/tenant/onboarding/views/ContactInformationStep.tsx` (`step: 'contact'`)
  - `src/modules/tenant/onboarding/views/VerificationStep.tsx` (`step: 'verification'`)
  - `src/modules/tenant/onboarding/views/SetPasswordView.tsx` (`step: 'password-setup'`)
- **Ne zaman**:
  - `contact`: İletişim formu gönderildiğinde (başarı veya hata)
  - `verification`: Başvuru tamamlama API’si (`completeTenantApplication`) başarılı veya hatalı olduğunda
  - `password-setup`: Şifre belirleme ekranında şifre kaydı başarılı veya hatalı olduğunda
- **Payload**:
  - **success**: `{ step: 'contact' | 'verification' | 'password-setup'; status: 'success' }`
  - **failed**: `{ step: 'contact' | 'verification' | 'password-setup'; status: 'failed'; http_status?: number | null; message?: string | null }`

### Tenant `/applications/new`

#### `store_application_step_completed`

- **Nerede**:
  - `src/modules/tenant/applications/create/context/StoreApplicationWizardContext.tsx`
- **Ne zaman**:
  - `location`: İlk adım validasyonu sonrası başarılı veya hatalı olduğunda
  - `store-details`: İkinci adım validasyonu sonrası başarılı veya hatalı olduğunda
  - `financial`: Final başvuru submit’i başarılı veya hatalı olduğunda
- **Payload**:
  - **success**: `{ step: 'location' | 'store-details' | 'financial'; status: 'success' }`
  - **failed**: `{ step: 'location' | 'store-details' | 'financial'; status: 'failed'; http_status?: number | null; message?: string | null }`
