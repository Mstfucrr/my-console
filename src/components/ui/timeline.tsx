...
const dotVariants = cva('flex h-4 w-4 items-center justify-center rounded-full', {
  variants: {
    color: {
  ...
  compoundVariants: [
    ...['destructive', 'success', 'info', 'warning', 'dark', 'secondary', 'primary'].map(color => ({
      variant: 'outline',
      color,
    ...
    return (
      <li
        ...
        className={cn(timelineItemVariants({ variant }), className, {
          '[&_.tm-content-wrapper]:pr-8 [&_.tm-content-wrapper]:text-right [&_.tm-separator]:right-0 [&_.tm-separator]:left-[unset]':
            position === 'left',
          'ltr:[&_.tm-content-wrapper]:pl-8 rtl:[&_.tm-content-wrapper]:pr-8 rtl:[&_.tm-content-wrapper]:text-right ltr:[&_.tm-separator]:left-0 rtl:[&_.tm-separator]:right-0 rtl:[&_.tm-separator]:left-[unset]':
            position === 'right' || position === undefined,
          'w-1/2': position === 'alternate' || position === 'alternate-reverse',
          'ml-auto rtl:mr-auto [&_.tm-content-wrapper]:pl-8 rtl:[&_.tm-content-wrapper]:pr-8':
            (position === 'alternate' && even) || (position === 'alternate-reverse' && odd),
          'rtl:mr-auto [&_.tm-content-wrapper]:pr-4 [&_.tm-content-wrapper]:text-right rtl:[&_.tm-content-wrapper]:pl-4 [&_.tm-separator]:right-[-17px] [&_.tm-separator]:left-[unset] rtl:[&_.tm-separator]:right-[-8px]':
            (position === 'alternate' && odd) || (position === 'alternate-reverse' && even),
  ...
  ({ className, children, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'tm-separator absolute top-[3px] flex h-full flex-col items-center',
...
interface TimelineDotProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof dotVariants> {
  children?: React.ReactNode
...
const TimelineDot = React.forwardRef<HTMLDivElement, TimelineDotProps>(
  ({ className, children, color, variant, ...props }, ref) => {
    return (
      <div ref={ref} className={cn(dotVariants({ color, variant }), className)} {...props}>
        <span>{children}</span>
      </div>
    )
  }
...
const TimelineConnector = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn('tm-connector bg-default-300 w-px grow', className)} {...props}>
        {children}
      </div>
    )
  }
