...
export function BottomNavigation() {
  ...
  return (
    <nav className='border-border bg-muted/40 ring-accent fixed bottom-1 left-1/2 z-50 w-[95%] max-w-max -translate-x-1/2 rounded-3xl border-t ring-2 backdrop-blur-2xl'>
      ...
      <div className='safe-area-inset-bottom'>
        <div className='xs:gap-x-3 relative flex items-center justify-between gap-x-2'>
          ...
          {visibleMenus.slice(2, 4).length > 0 && (
            <div className='xs:gap-x-3 flex flex-1 items-center justify-around gap-x-2'>
              {visibleMenus.slice(2, 4).map(item => (
                <BottomNavigationItem
                  key={item.title}
                  href={item.href}
                  label={item.title}
